...
;; Quest Data Table
;;
;; The basic quest data mechanism is based on a tbl of quest information. This
;; allows quests to be created once, then tracked and updated from a central
;; repository.  While not suitable for quests generated on the fly, this is a
;; lot more convenient for complex plot based quests. On the fly quests can
;; still interface directly with the quest-sys module.
;;
;; The questadd function here handles creation of quests (it also makes sure
;; the quest is created iff it is needed, so savegames have some chance of
;; updating right)
;;
;; It is inside a let definition, and hence wont work elsewhere. The quest to
;; be added is a quest using qst-mk from the quest-sys module
;;
;; (questadd (qst-mk
;;    "Name of Quest"
;;    'tag-to-refer-to-quest
;;    "Text description of quest, probably using kern-ui-paginate-text"
;;    'function-called-on-quest-assignment  ;; probably 'quest-assign-notify,
;;                                          ;;   or nil
...
;;	)
;;
;; The various quest-data-* methods assume that quest-payload is a tbl,
;; containing various info possibly including:
;;
;;     'on-update a method name that will be called in response to a
;;                quest-data-update call
;;
;;     'bonus-xp  a storage space for experience rewards that are accrued
;;                before the player knows about the quest (the xp will increase
;;                the rewards given once the the player knows why they are
;;                being given xp)
;;
...
;;
;; * adding a (quest-data-assign-once 'tag-to-refer-to-quest) at the relevent
;;   place in the game code
;;
;; * adding a (quest-data-complete 'tag-to-refer-to-quest) at the relevent
;;   place in the game code
;;              
;; * for nicely updating quest information, add the on-update method as
;;   described above, and sprinkle the plot with
;;
;;       (quest-data-update 'tag-to-refer-to-quest
;;                          'name-of-quest-flag
;;                           value-to-set-tag-to)
;;   and
;;       (quest-data-update-with 'tag-to-refer-to-quest
;;                               'name-of-quest-flag
;;                               value-to-set-tag-to
...
;;
;;       (quest-data-update-with 'tag-to-refer-to-quest
;;                               'name-of-quest-flag value-to-set-tag-to
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  (questadd (qst-mk "The Lost Toy"
		    'questentry-puska
		    (kern-ui-paginate-text "Ilya lost her toy while hiding in the basement from trogs on her parents homestead. "
