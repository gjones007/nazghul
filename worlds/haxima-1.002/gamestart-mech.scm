...
(define (one-off-message kbeing message messageid)
  (if (is-player-party-member? kbeing)
	...
	(map (lambda (trigobj)
	     ...
	     (kplace-get-objects-of-type (car (kern-obj-get-location kbeing))
					 t_step_trig))
	#f
	)))
...
(define (gamestart-reset-lamps kbeing)
  (let ((str (floor (+ (/ (* (- (kern-char-get-strength kbeing) 10) 7) 12) 1)))
	...
	(int (floor (+ (/ (* (- (kern-char-get-intelligence kbeing) 10) 7) 12)
		       1)))
...
(mk-obj-type 't_start_portal "path forward" s_blackgate_full layer-mechanism
	     step-trig-ifc)	
...
(define (start-actual-game kplayer)
  ...
  (kern-obj-relocate kplayer
		     (list p_moongate_clearing 11 12)
		     (mk-start-cutscene kplayer
					(get-gamestart-data kplayer 3)))
