...
bool Being::pathfindTo(struct place *destplace, int destx, int desty, int flags)
{
	...
	if (USE_CACHED_PATH && cachedPath) {
		...
		if (getPlace() != cachedPathPlace) {
			...
			if (pathPtr->x != getX() || pathPtr->y != getY()) {
				...
				if (pathPtr
				    && place_get_object(getPlace(), pathPtr->x,
							pathPtr->y,
							field_layer) != NULL) {
	...
	result = move(pathPtr->x - getX(), pathPtr->y - getY());
								     ...
								     being_layer)))
			{
				if (!are_hostile(this, occupant)
					...
					if (!place_is_passable
					    (getPlace(), getX(), getY(),
					     occupant, 0)) {
						relocate(getPlace(), pathPtr->x,
							 pathPtr->y);
						runHook(OBJ_HOOK_MOVE_DONE,
							"pdd", getPlace(),
							pathPtr->x, pathPtr->y);
	...
	if (result == WasImpassable && isOnMap()) {
		...
		mech = place_get_object(getPlace(),
					pathPtr->x, pathPtr->y, mech_layer);
		...
		if (WasImpassable == result && isOnMap()) {
			...
			cachedPath =
			    place_find_path(getPlace(), &as_info, this);
