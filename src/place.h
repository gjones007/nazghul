...
extern struct place *place_new(const char *tag,
			       ...
			       int wraps,
			       int underground,
			       int wilderness, int wilderness_combat);
...
extern int place_flying_distance(struct place *place, int x0, int y0, int x1,
				 int y1);

void place_get_direction_vector(struct place *place, int x1, int y1,
				int x2, int y2, int *dx, int *dy);

extern class Party *place_get_Party(struct place *place, int x, int y);

extern class Vehicle *place_get_vehicle(struct place *place, int x, int y);

extern int place_eval_path(struct place *place, int x0, int y0, int x1, int y1);

extern void place_move_object(struct place *place,
			      class Object * object, int newx, int newy);
...
static inline int place_off_map(struct place *place, int x, int y)
{
	if (place->wraps)
		return 0;
	return (x < 0 || x >= place_w(place) || y < 0 || y >= place_h(place));
}
...
{
	if (place->wraps)
		return ((x + place->terrain_map->w) % place->terrain_map->w);
	return x;
}
...
{
	if (place->wraps)
		return ((y + place->terrain_map->h) % place->terrain_map->h);
	return y;
}
...
extern void place_paint_objects(struct place *place, int mx, int my, int sx,
				int sy);
extern int place_los_blocked(struct place *place, int ax, int ay, int bx,
			     int by);
...
extern void place_for_each_object_at(struct place *place, int x, int y,
				     void (*fx) (class Object *, void *),
				     void *);
...
extern int place_get_edge_entrance(struct place *place, int dir, int *x,
				   int *y);
extern int place_set_edge_entrance(struct place *place, int dir, int x, int y);
extern class Object *place_get_filtered_object(struct place *place, int x,
					       int y,
					       int (*filter) (class Object *));
extern struct place *place_get_neighbor(struct place *place, int dir);
extern void place_set_neighbor(struct place *place, int dir,
			       struct place *neighbor);
extern int place_in_los(struct place *p1, int x1, int y1, struct place *p2,
			int x2, int y2);
