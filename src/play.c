...
static void play_loop(void)
{
        ...
        while (1) {
                ...
                session_run_hook(Session, turn_start_hook, "p",
                                 Session->player);
...
static int play_load_session(char *fname)
{
        ...
        log_end("^c+gok^c-!");
        assert(Session);
        log_msg("Save file version is %u.%u.%u", Session->major, Session->minor,
                Session->release);
        foogodSetMode(FOOGOD_DEFAULT);
        session_run_hook(Session, new_game_start_hook, "p", Session->player);
        return 0;
