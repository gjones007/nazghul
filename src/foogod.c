...
static void foogod_repaint_title(void)
{
	...
	screen_print(&Foogod.titleRect, SP_CENTERED | SP_ONBORDER, "%s",
		     Foogod.title);
...
static int foogod_load_progress_bar_sprites(void)
{
	...
	for (i = 0; i < 3; i++) {
		if (!
		    (Foogod.progress_bar_sprites[i] =
		     sprite_new(0, 1, i, 0, 0, Foogod.image))) {
...
static void foogodPaintEffects()
{
	...
	for (int i = 0; i < OBJ_NUM_HOOKS; i++) {
		player_party->hookForEach(i, foogod_paint_effect_wrapper,
					  &rect);
	}
