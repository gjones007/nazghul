...
void ctrl_party_ai(class Party * party)
{
        ...
        dbg("%s:%s@%s[%d %d]\n", __FUNCTION__, party->getName(),
            party->getPlace()->name, party->getX(), party->getY());
        ...
        dbg("%s:player@%s[%d %d]\n", __FUNCTION__,
            player_party->getPlace()->name, player_party->getX(),
            player_party->getY());
        ...
        if (party->getX() == target_x && party->getY() == target_y) {
                 ...
                 * map. */
                if (player_place != party->getPlace()) {
                        combat_add_party_on_edge(party, party->getDx(),
                                                 party->getDy(), player_place);
                        return;
                }
        ...
        if (d > 1 && (Place == party->getPlace())       /* don't attack towns or temp combat maps */
...
static int ctrl_calc_to_hit(class Character * character,
                            class ArmsType * weapon, int penalty)
{
...
static int ctrl_calc_damage(class Character * character,
                            class ArmsType * weapon, char critical)
{
...
static void ctrl_attack_done(class Character * character,
                             class ArmsType * weapon, class Character * target)
{
...
static class Character *ctrl_get_interfering_hostile(class Character *
                                                     character)
{
        ...
        for (i = 0; i < 4; i++) {
                near =
                    (class Character *) place_get_object(character->getPlace(),
                                                         character->getX() +
                                                         dx_to_neighbor[i],
                                                         character->getY() +
                                                         dy_to_neighbor[i],
                ...
                if (near &&
                    are_hostile(near, character) && !near->isIncapacitated()) {
                        return near;
                }
        }
...
static void ctrl_suggest_visitor(class Object * obj, void *data)
{
        struct nearest_hostile_info *info =
            (struct nearest_hostile_info *) data;
        ...
        if (!place_in_los(info->origin->getPlace(),
                          info->origin->getX(),
                          info->origin->getY(),
                          info->origin->getPlace(), obj->getX(), obj->getY()))
                return;

        if (info->range <
            (dist =
             place_flying_distance(info->origin->getPlace(),
                                   info->origin->getX(), info->origin->getY(),
                                   obj->getX(), obj->getY())))
                return;
		...
		if (target == NULL) {
			target = ctrl_get_flyer(weapon->getRange(),
                ...
                if (target == NULL) {
                        ...
                        /* Attack the terrain */
                        terrain =
                            place_get_lower_terrain(character->getPlace(), x, y,
                                                    NULL);
                        ...
                        weapon->fireHitLoc(character, NULL,
                                           character->getPlace(), misx, misy,
                                           -1);
                        ...
                        if (!miss) {
                                /* Check for a mech */
                                mech =
                                    place_get_object(character->getPlace(), x,
                                                     y, mech_layer);
        ...
        case WasImpassable:
                {
                        ...
                        mech =
                            place_get_object(character->getPlace(), newx, newy,
                                             mech_layer);

                        if (mech && mech->getObjectType()->canHandle()) {
                                mech->getObjectType()->handle(mech, character);
                                character->decrementActionPoints(kern_intvar_get
                                                                 ("AP_COST:handle_mechanism"));
                                mapSetDirty();
                                result = "handled!";
                                      ...
                                      int keymod)
{
        ...
        case 'f':
                if (cmdToggleFollowMode()) {
                        ...
                        foogod_set_title("Round Robin: %s",
                                         character->getName());
                        foogodRepaint();
                }
                break;
                                    ...
                                    class Character * target)
{
        ...
        for (class ArmsType * weapon = character->enumerateWeapons(&armsIndex);
             weapon != NULL; weapon = character->getNextWeapon(&armsIndex)) {
                ...
                if (character->hasAmmo(weapon) && !weapon->isMissileWeapon()) {
                        return 0;
                }
        }
                               ...
                               class Character * target)
{
        ...
        for (class ArmsType * weapon = character->enumerateWeapons(&armsIndex);
             weapon != NULL; weapon = character->getNextWeapon(&armsIndex)) {
                ...
                if (weapon->obstructed(character->getPlace(), character->getX(),
                                       character->getY(), target->getX(),
                                       target->getY())) {
                        continue;
                }
