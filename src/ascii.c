				  ...
				  Uint32 color)
{
	...
	for (y = 0; y < dstrect->h; y++) {
		...
		srcpix = (Uint16 *) src->pixels
		    + (srcrect->y + y) * src->w + srcrect->x;
		dstpix = (Uint16 *) dst->pixels
		    + (dstrect->y + y) * dst->w + dstrect->x;
				...
				SDL_Surface * surf, Uint32 color)
{
	...
	ascii_blit_colored(Ascii.images->images, &src, surf, &dest, color);
}
...
void ascii_paint_glyph(glyph_t glyph, int x, int y, SDL_Surface * surf)
{
	...
	if ('w' == color) {
		...
		ascii_paint_colored(cc, x, y, surf, ascii_decode_color(color));
	}
}
