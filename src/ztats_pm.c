...
static void ztats_pm_paint_arms(SDL_Rect * rect, ArmsType * arms)
{
	...
	screen_print(rect, 0, "%s  (Rng:%d, AP:%d, Spd:%d)",
		     arms->getName(), arms->getRange(),
		     arms->getRequiredActionPoints(), arms->get_AP_mod());
	rect->y += ASCII_H;
...
void ztats_pm_enter(struct ztats_pane *pane, class Party * party,
...
void ztats_pm_paint(struct ztats_pane *pane)
{
	...
	screen_print(&rect, 0,
		     ...
		     STAT_LABEL_CLR,
		     status_range_color(ch->getActionPoints(),
					ch->getActionPointsPerTurn()),
	...
	screen_print(&rect, 0,
		     "^c%cSpecies:    ^cw%s",
		     STAT_LABEL_CLR,
		     ch->species ? ch->species->name : "Unknown");
	rect.y += ASCII_H;
	...
	screen_print(&rect, 0,
		     "^c%cMovement:   ^cw%s", STAT_LABEL_CLR, mmode->name);
	rect.y += ASCII_H;
	rect.y += ASCII_H;
	...
	/* Show arms */
	screen_print(&rect, 0 /*SP_CENTERED */ , "^c%c*** Arms ***^cw",
		     STAT_LABEL_CLR);
	rect.y += ASCII_H;
