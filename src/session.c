...
void load_err(const char *fmt, ...)
{
        ...
        va_start(args, fmt);
        vsnprintf(session_last_error, sizeof (session_last_error), fmt, args);
        va_end(args);
...
void *session_add(struct session *session, void *obj,
                  void (*dtor) (void *),
                  void (*save) (save_t *, void *), void (*start) (void *)
    )
{
...
struct session *session_new(void *interp)
{
        struct session *session =
            (struct session *) calloc(1, sizeof (*session));
        assert(session);
...
void session_del(struct session *session)
{
        ...
        if (!node_list_empty(&session->sched_chars)) {
                fprintf(stderr, "warn: session sched_chars list non-empty\n");
        }
...
int session_load(char *filename)
{
        ...
        sky_start_session(&Session->sky, NULL != Place && !Place->underground);
...
int session_save(char *fname)
{
        ...
        save->write(save, "(define original-load load)  "
                    "(define (load file) "
                    "(kern-progress-bar-advance 1) " "(original-load file))\n");
                      ...
                      const char *fmt, ...)
{
        ...
        list_for_each(head, lptr) {
                struct session_hook_entry *entry =
                    list_entry(lptr, struct session_hook_entry, list);
                va_list args;
                va_start(args, fmt);
...
static void session_cleanup_hooks(struct session *session)
{
        ...
        for (i = 0; i < NUM_HOOKS; i++) {
                ...
                while (lptr != head) {
                        ...
                        lptr = lptr->next;
                        list_remove(&entry->list);
                        entry->proc->sc->vptr->unprotect(entry->proc->sc,
                                                         entry->args);
