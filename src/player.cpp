...
enum move_result PlayerParty::check_move_to(struct move_info *info)
{
	...
	if (!place_move_is_passable
	    (info->place, info->x - info->dx, info->y - info->dy, info->x,
	     info->y, this, PFLAG_MOVEATTEMPT))
...
enum MoveResult PlayerParty::
try_to_enter_subplace_from_edge(struct place *subplace, int dx, int dy)
{
	int new_x;
	int new_y;
	int dir;
...
enum MoveResult PlayerParty::try_to_move_off_map(struct move_info *info)
{
	...
	relocate(info->place->location.place,
		 info->place->location.x, info->place->location.y);
	...
	case move_ok:
		{
			...
			mv_cost =
			    place_get_diagonal_movement_cost(info.place, x, y,
							     info.x, info.y,
							     this, 0);
...
void PlayerParty::removeMember(class Character * c)
{
	...
	for (entry = node_next(c->plnode), index = c->getOrder();
	     entry != &members; entry = node_next(entry), index++) {
...
void PlayerParty::beginCamping(class Character * guard, int hours)
{
	...
	if (camp_guard && !camp_guard->isDead()
	    && !camp_guard->isAsleep())
...
bool PlayerParty::rendezvous(struct place *place, int rx, int ry)
{
	...
	FOR_EACH_MEMBER(entry, member) {
		...
		member->cachedPath = place_find_path(place, &as_info, member);
		...
		if (!member->cachedPath) {
			...
			log_msg("%s is too far away!", member->getName());
			abort = true;
		}
	}
	...
	while (!done) {
		...
		FOR_EACH_MEMBER(entry, member) {
			...
			// move one step
			member->move(member->cachedPath->next->x -
				     member->getX(),
				     member->cachedPath->next->y -
				     member->getY());
...
void PlayerParty::sortReadiedItems(class Character * member)
{
	struct inv_entry *ie;
	int armsIndex = 0;
	for (class ArmsType * weapon = member->enumerateArms(&armsIndex);
	     weapon != NULL; weapon = member->getNextArms(&armsIndex)) {
