...
void astral_body_save(struct astral_body *body, struct save *save)
{
	...
	for (i = 0; i < body->n_phases; i++) {
		save->write(save, "(list %s %d \"%s\")\n",
			    ...
			    body->phases[i].maxlight, body->phases[i].name);
	}
	save->exit(save, ")\n");
	save->exit(save, ")\n");
...
static void astral_body_advance_arc(struct astral_body *body)
{
	...
	body->light = sky_get_light_from_astral_body(body->arc,
...
static void sky_get_eclipse(struct sky *sky, struct astral_body *outer)
{
