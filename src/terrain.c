...
struct terrain *terrain_new(const char *tag,
                            const char *name, struct sprite *sprite, int pclass)
{
...
/**
 * Return the entry for `glyph` in `palette`, or NULL if not found.
 */
static struct terrain_palette_entry *palette_get(struct terrain_palette
                                                 *palette, char *glyph)
{
        struct list *elem;
        struct terrain_palette_entry *entry;
...
int palette_terrain_is_null_glyph(struct terrain_palette *palette, char *glyph)
{
        struct terrain_palette_entry *entry;
        entry = palette_get(palette, glyph);
