...
void combat_set_state(enum combat_state new_state)
{
        ...
        case COMBAT_STATE_DONE:
                ...
                case COMBAT_STATE_FIGHTING:
                        ...
                        session_run_hook(Session, combat_change_hook, "py",
...
static int combat_find_safe_position(struct position_info *info)
{
        ...
        while (q_head != q_tail) {
                ...
                newsafety =
                    combat_search_for_safe_position(info, currentsafety);
                if (newsafety == 0)
                        return 0;
                else if (newsafety != -1) {
...
void combat_fill_position_info(struct position_info *info, struct place *place,
                               int x, int y, int dx, int dy, bool defend)
{
        // 
        // This function will:
        ...
        if (info->place != Combat.place) {
                ...
                if (dx < 0) {
                        ...
                        info->x =
                            place_w(info->place) - place_w(info->place) / 4;
                } else if (dx > 0) {
                ...
                if (dy < 0) {
                        ...
                        info->y =
                            place_h(info->place) - place_h(info->place) / 4;
                } else if (dy > 0) {
                                   ...
                                   struct position_info *pinfo)
{
        ...
        if (dx < 0) {
                ...
                set_party_initial_position(pinfo, (COMBAT_MAP_W * 3) / 4,
                                           COMBAT_MAP_H / 2);
        } else if (dx > 0) {
                // facing east, fill west half
                dst_x = COMBAT_MAP_W / 2 - COMBAT_MAP_W;
                set_party_initial_position(pinfo, COMBAT_MAP_W / 4,
                                           COMBAT_MAP_H / 2);
        } else if (dy < 0) {
                // facing north, fill south half
                dst_y = COMBAT_MAP_H / 2;
                set_party_initial_position(pinfo, COMBAT_MAP_W / 2,
                                           (COMBAT_MAP_H * 3) / 4);
        } else if (dy > 0) {
                               ...
                               struct position_info *pinfo, int broadside)
{
        ...
        if (broadside) {
                terrain_map_rotate(map,
                                   vector_to_rotation(pinfo->dy, pinfo->dx));
        ...
        // map.
        set_party_initial_position(pinfo, x + (map->w /*+ 1 */ ) / 2,
                                   y + (map->h /*+ 1 */ ) / 2);
...
static enum combat_faction_status combat_get_hostile_faction_status(void)
{
        ...
        node_foldr(place_get_all_objects(Place),
                   combat_hostile_status_visitor, &stat);
...
static enum combat_faction_status combat_get_player_faction_status(void)
{
         ...
         * the player faction. */
        node_foldr(place_get_all_objects(Place),
                   combat_player_status_visitor, &stat);
...
static void setup_combat_place_part(struct place *place,
                                    struct terrain *our_terrain,
                                    struct terrain *other_terrain, int dx,
                                    int dy, int mapx, int mapy)
{
        int dst_x = 0, dst_y = 0;
        ...
        if (our_terrain->renderCombat) {
                ...
                terrain_map_fill(place->terrain_map, 0, 0, COMBAT_MAP_W,
                                 COMBAT_MAP_H, our_terrain);
        }
}
                               ...
                               *info)
{
        ...
        player_terrain =
            place_get_terrain(player_party->getPlace(), info->pc_x, info->pc_y);

        if (info->move->npc_party) {
                npc_terrain =
                    place_get_terrain(info->move->npc_party->getPlace(),
                                      info->npc_x, info->npc_y);
        } else {
                npc_terrain = player_terrain;   // some sensible definition makes like easier...
        }

        setup_combat_place_part(place, player_terrain, npc_terrain, info->pc_dx,
                                info->pc_dy, info->pc_x, info->pc_y);
        if (info->move->npc_party) {
                setup_combat_place_part(place, npc_terrain, player_terrain,
                                        -info->pc_dx, -info->pc_dy, info->npc_x,
                                        info->npc_y);
        }
...
static bool position_player_party(struct combat_info *cinfo)
{
        ...
        if (vehicle &&
            vehicle->getObjectType()->renderCombat && Place == Combat.place) {
...
bool combat_enter(struct combat_info * info)
{
        ...
        if (!info->move->place->wilderness) {
                ...
                Combat.place = place_new("p_wilderness_combat", "Wilderness Combat", 0, // sprite
                                         create_temporary_terrain_map(info), 0, // ! wrapping
                                         info->move->place->underground, 0,     // ! wilderness
                                         1      // wilderness combat
                    );
 ...
 */
int combat_add_party_on_edge(class Party * party, int dx, int dy,
                             struct place *place)
{
        struct position_info pinfo;
        ...
        pinfo.place = place;
        pinfo.x =
            dx > 0 ? place_left(place) : (dx <
                                          0 ? place_right(place) :
                                          place_horz_center(place));
        pinfo.y =
            dy > 0 ? place_top(place) : (dy <
                                         0 ? place_bottom(place) :
                                         place_vert_center(place));
        pinfo.dx = dx;
        pinfo.dy = dy;
        pinfo.rx = 0;
        pinfo.ry = 0;
