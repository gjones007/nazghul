...
void ztats_container_pane_enter(struct ztats_pane *pane, class Party * party,
...
int ztats_container_pane_scroll(struct ztats_pane *pane,
                                enum StatusScrollDir dir)
{
        ...
        case ScrollPageDown:
                zcp->top_line =
                    min(zcp->max_line, zcp->top_line + zcp->num_lines);
                break;
        case ScrollLeft:
        case ScrollRight:
        default:
...
void ztats_container_pane_paint(struct ztats_pane *pane)
{
        ...
        for (ie = container->first(zcp->filter); ie != NULL;
             ie = container->next(ie, zcp->filter)) {
...
void ztats_container_pane_init(struct ztats_container_pane *zcp,
