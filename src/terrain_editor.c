...
struct terrain_editor_pane {
        ...
        int (*key_handler) (struct terrain_editor_pane * pane, int key,
                            int keymod);
         bool(*mouse_click_handler) (struct terrain_editor_pane * pane, int sx,
                                     int sy);
};
...
{
        log_msg("[%s]: %s '%s'", msg, tt ? tt->tag : NULL,
                tt ? tt->name : NULL);
}
...
static void terrain_editor_cmd_fill_exec(struct terrain *nt, struct terrain *ot,
...
static void terrain_editor_cmd_exec(struct terrain_editor_applet *tea,
                                    struct terrain_editor_cmd *cmd)
{
        switch (cmd->code) {
        case CMD_PAINT:
                {
        ...
        case CMD_FILL:
                {
                        ...
                        terrain_editor_cmd_fill_exec(newter, oldter, tea->place,
                                                     x, y);
                }
                break;
        default:
...
static struct terrain_editor_cmd *terrain_editor_cmd_paint_new(int x, int y, struct terrain
                                                               *terrain)
{
        struct terrain_editor_cmd *cmd =
            (struct terrain_editor_cmd *) calloc(1, sizeof (*cmd));
        if (!cmd) {
                return NULL;
        }
        cmd->code = CMD_PAINT;
...
static struct terrain_editor_cmd *terrain_editor_cmd_fill_new(int x, int y, struct terrain
                                                              *oldter, struct terrain
                                                              *newter)
{
        struct terrain_editor_cmd *cmd =
            (struct terrain_editor_cmd *) calloc(1, sizeof (*cmd));
...
static void terrain_editor_del_cmd_list(struct terrain_editor_applet *tea,
                                        struct list *elem)
{
        while (elem != &tea->cmds) {
                struct terrain_editor_cmd *cmd =
                    list_entry(elem, struct terrain_editor_cmd, list);
                elem = elem->next;
...
static void terrain_editor_submit_cmd(struct terrain_editor_applet *tea,
                                      struct terrain_editor_cmd *cmd)
{
...
static void terrain_editor_undo(struct terrain_editor_applet *tea)
{
        ...
        tea->place->terrain_map =
            terrain_map_clone(tea->origmap, tea->origmap->tag);
...
static void terrain_editor_redo(struct terrain_editor_applet *tea)
{
        ...
        /* replay last command */
        struct terrain_editor_cmd *cmd =
            list_entry(tea->lastcmd, struct terrain_editor_cmd, list);
        terrain_editor_cmd_exec(tea, cmd);
 ...
 */
static void terrain_editor_req_paint(struct terrain_editor_applet *tea, int x,
                                     int y)
{
        if (tea->terrain != place_get_terrain(tea->place, x, y)) {
                struct terrain_editor_cmd *cmd =
                    terrain_editor_cmd_paint_new(x, y, tea->terrain);
 ...
 */
static void terrain_editor_req_fill(struct terrain_editor_applet *tea, int x,
                                    int y)
{
        struct terrain *oldter = place_get_terrain(tea->place, x, y);
        if (tea->terrain != oldter) {
                struct terrain_editor_cmd *cmd =
                    terrain_editor_cmd_fill_new(x, y, oldter, tea->terrain);
                terrain_editor_submit_cmd(tea, cmd);
        }
}
 ...
 */
static void terrain_editor_look_at_xy(struct terrain_editor_applet *tea, int x,
                                      int y)
{
...
static int terrain_editor_palette_key_handler(struct terrain_editor_pane *pane,
                                              int key, int keymod)
{
 ...
 */
static int terrain_editor_key_handler(struct KeyHandler *kh, int key,
                                      int keymod)
{
        ...
        if (key >= '0' && key <= '9') {
                ...
                if ((keymod && KMOD_LCTRL) || (keymod && KMOD_RCTRL)) {
                        ...
                        log_msg("[Set Quick %d]: %s '%s'", qt,
                                tea->terrain->tag, tea->terrain->name);
                        return 0;
                } else {
        ...
        case KEY_SHIFT_EAST:
                {
                        ...
                        tea->pane =
                            outcast(list, struct terrain_editor_pane, list);
                        statusRepaint();
                }
                break;
        ...
        case 'f':
                terrain_editor_req_fill(tea, session->crosshair->getX(),
                                        session->crosshair->getY());
 ...
 * clicked.
 */
static bool terrain_editor_map_click(struct terrain_editor_applet *tea, int mx,
                                     int my)
{
#if 1
        struct session *session = tea->base.session;
 ...
 */
static terrain *terrain_editor_next(struct terrain_editor_applet *tea,
                                    struct terrain *ter)
{
 ...
 * list.
 */
static terrain *terrain_editor_lookup(struct terrain_editor_applet *tea,
                                      int index)
{
        struct list *head = &tea->base.session->terrains;
 ...
 * coordinates clicked.
 */
static bool terrain_editor_palette_mouse_click_handler(struct
                                                       terrain_editor_pane
                                                       *pane, int pane_x,
                                                       int pane_y)
{
 ...
 * terraform mode
 */
static bool terrain_editor_mouse_button_handler(struct MouseButtonHandler *mh,
                                                SDL_MouseButtonEvent * event)
{
        ...
        if (point_in_rect(event->x, event->y, &tea->base.dims)) {
                tea->pane->mouse_click_handler(tea->pane,
                                               event->x - tea->base.dims.x,
                                               event->y - tea->base.dims.y);
        }
 ...
 * terraform mode
 */
static bool terrain_editor_mouse_motion_handler(struct MouseMotionHandler *mh,
 ...
 */
static void terrain_editor_applet_ops_run(struct applet *applet,
                                          SDL_Rect * dims,
                                          struct session *session)
{
        ...
        tea->terrain =
            list_entry(session->terrains.next, struct terrain, session_list);
        tea->pal->max_top_index =
            list_len(&session->terrains) -
            (tea->pal->max_cols * tea->pal->max_rows);
...
static void terrain_editor_applet_del(struct terrain_editor_applet *tea)
{
        ...
        while (list != &tea->panes) {
                struct terrain_editor_pane *pane =
                    outcast(list, struct terrain_editor_pane, list);
                list = list->next;
                free(pane);
        }
...
static struct terrain_editor_applet *terrain_editor_applet_new(struct place
                                                               *place, int x,
                                                               int y)
{
        ...
        struct terrain_editor_applet *tea =
            (struct terrain_editor_applet *) calloc(1, sizeof (*tea));
        if (!tea) {
                return NULL;
        }
        ...
        if (!
            (tea->origmap =
             terrain_map_clone(tea->place->terrain_map,
                               tea->place->terrain_map->tag))) {
                goto abort;
        }
        ...
        tea->pal =
            (struct terrain_editor_palette *) calloc(1, sizeof (*(tea->pal)));
        if (!tea->pal) {
                goto abort;
        }
        ...
        tea->pal->base.mouse_click_handler =
            terrain_editor_palette_mouse_click_handler;
...
{
        struct terrain_editor_applet *tea =
            terrain_editor_applet_new(place, x, y);
        if (tea) {
