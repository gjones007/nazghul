 ...
 * Copyright (C) 2002, 2003, 2015 Gordon McNutt
 *
...
bool mouse_button_cursor(struct MouseButtonHandler * mh,
                         SDL_MouseButtonEvent * event)
{
        struct movecursor_data *data = (struct movecursor_data *) mh->data;
        int mx = event->x;
        int my = event->y;
        ...
        if (event->button == SDL_BUTTON_LEFT) {
                if (data->each_target_func) {
                        return data->each_target_func(Session->crosshair->
                                                      getPlace(),
                                                      Session->crosshair->
                                                      getX(),
                                                      Session->crosshair->
...
bool mouse_motion_cursor(struct MouseMotionHandler * mh,
                         SDL_MouseMotionEvent * event)
{
        ...
        if (data->each_tile_func) {
                data->each_tile_func(Session->crosshair->getPlace(),
                                     Session->crosshair->getX(),
                                     Session->crosshair->getY(), data->data);
        }
...
{
        int moved = 0;
        struct movecursor_data *data = (struct movecursor_data *) kh->data;
        ...
        if (keyIsDirection(key)) {
                ...
                case 'n':
                        /* Next target */
                        if (data->loc_list && !list_empty(data->loc_list)) {
...
void getkey(void *data,
            int (*handler) (struct KeyHandler * kh, int key, int keymod))
{
        struct KeyHandler kh;
        kh.fx = handler;
        kh.data = data;
...
{
        return (int) (obj->getObjectType() && obj->getObjectType()->canGet());
}
...
bool cmdOpen(class Character * pc)
{
        ...
        if (mech && mech->getObjectType()->canOpen()) {
                ...
                pc->decrementActionPoints(kern_intvar_get
                                          ("AP_COST:open_mechanism"));
                return true;
        }
...
void cmdFire(void)
{
        ...
        if ((!vehicle || !vehicle->getOrdnance())) {
                // SAM: 
        ...
        if (!vehicle->fire_weapon(directionToDx(dir),
                                  directionToDy(dir), player_party)) {
...
bool cmdReady(class Character * member)
{
        ...
        for (;;) {
                ...
                //do not allow too much AP debt if in combat
                if ((combat_get_state() == COMBAT_STATE_FIGHTING)
                    && ((2 * member->getActionPoints()) + member->getSpeed() <
                        0)) {
                        break;
                }
                             ...
                             i_fncptr_iiv_t each_target_func)
{
        return select_target_rlcb(Place, ox, oy, x, y, range, 0,
                                  each_tile_func, each_target_func);
}
...
/**
 * Check if `obj` can be talked to.
 *
 * `data` must be a pointer to a talk_info struct. If `obj` can be talked to,
 * it will be added to the `suggest` list in `data`. If it is nearer than any
 * other suggestion, it will be the new `nearest`.
 */
static void cmd_talk_info_visitor(class Object * obj, void *data)
{
        struct location_list *entry = 0;
        int dist = 0;
        struct talk_info *info = (struct talk_info *) data;
        ...
        if (!info->nearest || dist < info->min_distance) {
                info->nearest = obj;
...
/**
 * Store a list of possible conversants within `range` into `info` and return
 * the one nearest to `member`.
 */
static class Object *cmd_get_talk_info(struct talk_info *info,
                                       class Object * member, int range)
{
        memset(info, 0, sizeof (*info));
        info->origin = member;
        info->nearest = NULL;
        ...
        place_for_each_object(member->getPlace(), cmd_talk_info_visitor, info);
...
void cmdTalk(Object * member)
{
        ...
        /* Start the cursor on the nearest conversant or, if none are
         * available, the member. */
        class Object *conversant = NULL;
        if (!TimeStop) {
        ...
        class Object *obj;
        obj = place_get_lower_object(Place, x, y, being_layer, NULL);
...
static int select_hours(int allow_sunrise)
{
        ...
        if (!info.c || info.c == '0') {
                ...
                cmdwin_spush("none!");
                return 0;
        } else if (allow_sunrise && (info.c == 's' || info.c == 'S')) {
                int hour;
                int sunrise;
...
bool cmdCastSpell(class Character * pc)
{
        ...
        magic_spell_code_to_name(&Session->magic, spell_name,
                                 MAX_SPELL_NAME_LENGTH, context.spell_name);
...
bool cmdMixReagents(class Character * character)
{
        ...
        if (character) {
                ...
                if (!spell) {
                        ...
                        int num =
                            kern_intvar_get("AP_COST:mix_reagents_nospell_num");
                        int dice =
                            kern_intvar_get
                            ("AP_COST:mix_reagents_nospell_dice");
                        int plus =
                            kern_intvar_get
                            ("AP_COST:mix_reagents_nospell_plus");
                        ...
                        int num =
                            kern_intvar_get("AP_COST:mix_reagents_badmix_num");
                        int dice =
                            kern_intvar_get("AP_COST:mix_reagents_badmix_dice");
                        int plus =
                            kern_intvar_get("AP_COST:mix_reagents_badmix_plus");
                        int AP_wasted =
                            dice_roll_numeric(level * num, dice, plus);
                        character->decrementActionPoints(AP_wasted);
                        ...
                        int per_mix =
                            kern_intvar_get("AP_COST:mix_reagents_per_mix");
                        int per_level =
                            kern_intvar_get("AP_COST:mix_reagents_per_level");
                        int AP_spent =
                            base + (quantity * per_mix) +
                            (spell->level * per_level);
...
bool cmdAT(class Character * pc)
{
        ...
        if (Place->underground) {
                log_msg("%s is underground, and cannot see the sky.", who);
        }                       // underground
        else {
                struct list *elem;
        ...
        if ((subplace = place_get_subplace(player_party->getPlace(),
                ...
                if (ENABLE_TOWN_ZOOM_IN) {
                        ...
                        log_msg("Enter-Use a side entrance!");
                                      ...
                                      player_party->getX(),
                                      player_party->getY(),
                                      player_party, PFLAG_IGNOREVEHICLES)) {
...
static int cmd_paint_skill(struct stat_super_generic_data *self,
                           struct node *node, SDL_Rect * rect)
{
            ...
            || !list_empty(&skill->materials)) {
                ...
                if (rect->h < ASCII_H) {
                        ...
                        node_for_each(&skill->tools, tnode) {
                                ...
                                class ObjectType *tool =
                                    (class ObjectType *) tnode->ptr;
                                struct inv_entry *ie =
                                    player_party->inventory->search(tool);
                        ...
                        list_for_each(&skill->materials, elem) {
                                ...
                                struct skill_material *mat =
                                    list_entry(elem, struct skill_material,
                                               list);
                                class ObjectType *objtype =
                                    (class ObjectType *) mat->objtype;
                                struct inv_entry *ie =
                                    player_party->inventory->search(objtype);
                                char mat_clr = ie ? 'g' : 'r';
                                char q_clr = (ie
                                              && (ie->count >=
                                                  mat->quantity)) ? 'g' : 'r';
                                screen_print(rect, 0,
                                             "^c+%c%s^c+%c (%d/%d)^c-^c-",
                                             mat_clr, objtype->getName(), q_clr,
                                             mat->quantity, ie ? ie->count : 0);
...
void cmdYuse(class Character * actor)
{
        ...
        /* check wilderness */
        if (!skill->wilderness_ok && place_is_wilderness(actor->getPlace())) {
                cant = 1;
        ...
        /* check special */
        if (skill->can_yuse && !closure_exec(skill->can_yuse, "p", actor)) {
                cant = 1;
        }
...
bool cmdSetSoloMode(int party_member_index)
{
        class Character *solo_member =
            player_party->getMemberAtIndex(party_member_index);
        if (solo_member && !solo_member->isIncapacitated()
            && solo_member->isOnMap()
            && solo_member->isPlayerControlled()
            ) {

                if (solo_member->engagedInTask()) {
                        log_msg("%s is engaged in %s, abort?",
                                solo_member->getName(),
                                solo_member->getTaskName());
