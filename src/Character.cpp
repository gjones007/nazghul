...
Character::Character(const char *tag, const char *name,
		     ...
		     int mpmod, int mpmult, int hp, int xp_, int mp, int lvl)
 : 
hm(0), xp(xp_), order(-1),
sleeping(false),
ac(0),
	...
	description(NULL)
{
	...
	if (xp < 0) {
		if (this->lvl == 1) {
			...
			this->xp =
			    getXpForLevel(this->lvl) +
			    (rand() % getXpForLevel(this->lvl));
		}
	}
}
...
fleeY(0),
fleePathFlags(0),
currentMmode(0), known(false), taskname(NULL), taskproc(NULL), taskgob(NULL)
{
			    ...
			    struct closure *cutscene)
{
	...
	if (vehicle && (!place_is_wilderness(dest_place)
...
enum MoveResult Character::move(int dx, int dy)
{
	...
	if (!place_move_is_passable
	    (getPlace(), getX(), getY(), newx, newy, this,
	     PFLAG_MOVEATTEMPT | (getActivity() ==
				  COMMUTING ? PFLAG_IGNOREMECHS : 0))) {
...
class ArmsType *Character::getNextArms(int *armsIndex)
{
	...
	*armsIndex = *armsIndex + 1;
	...
	for (; *armsIndex < species->n_slots; *armsIndex = *armsIndex + 1) {
		...
		if (*armsIndex > 0
		    && rdyArms[*armsIndex - 1] == rdyArms[*armsIndex]
...
{
	if (slot < 0 || slot >= species->n_slots)
		return NULL;
	return rdyArms[slot];
}
...
bool Character::hasInInventory(class ObjectType * type)
{
	if (isPlayerControlled()) {
		...
		return (container != NULL && container->search(type) != NULL);
	}
}
...
{
	if (fleePathFound && followFleePath()) {
		return true;
	}
...
bool Character::initCommon(void)
{
	if (species && species->n_slots > 0) {
		rdyArms =
		    (class ArmsType **) calloc(species->n_slots,
...
void Character::resurrect(void)
{
	...
	putOnMap(player_party->get_leader()->getPlace(),
		 ...
		 player_party->get_leader()->getY(), 4, 0);
...
class Object *Character::clone()
{
	...
	int armsIndex = 0;
	for (ArmsType * arms = enumerateArms(&armsIndex); arms != NULL;
...
void Character::examine()
{
	...
	log_continue(" [");
	for (ArmsType * arms = enumerateArms(&i); arms; arms = getNextArms(&i)) {
		if (n > 0) {
			log_continue(", ");
		}
...
int Character::getBaseAttackBonus(class ArmsType * weapon)
{
	int strbonus =
	    weapon->
	    modifyStrAttack(session_run_query
			    (Session, str_based_attack_query, "p", this));
	int dexbonus =
	    weapon->
	    modifyDexAttack(session_run_query
			    (Session, dex_based_attack_query, "p", this));
...
int Character::getDamageBonus(class ArmsType * weapon)
{
	int dambonus =
	    weapon->
	    modifyDamageBonus(session_run_query
			      (Session, damage_bonus_query, "p",
			       this)) / (100 * 1000);
...
int Character::getAvoidBonus()
{
	...
	int avoidBonus =
	    session_run_query(Session, defense_bonus_query, "p", this);
	float avoidMod = 1;
...
{
	return (place_flying_distance(getPlace(), getX(), getY(),
				      obj->getX(), obj->getY()) <=
		getVisionRadius() &&
		obj->isVisible() &&
...
bool Character::atAppointment()
{
	...
	if (getX() >= curAppt->x && getX() < (curAppt->x + curAppt->w)
	    && getY() >= curAppt->y && getY() < (curAppt->y + curAppt->h)) {
		return true;
	}
	return false;
}
...
bool Character::playerIsInMyBed()
{
	...
	return (sleeper && sleeper->isPlayerControlled()
		&& sleeper->isResting());
}
...
bool Character::commute()
{
	...
	if (playerIsInMyBed()) {
		if (nextToAppointment()) {
			...
			pathfindTo(getPlace(), curAppt->x, curAppt->y,
				   PFLAG_IGNOREBEINGS | PFLAG_IGNOREMECHS);
		}
		return true;
	}
	...
	dbg("%s cannot find path to [%d %d %d %d] while commuting\n",
	    getName(),
	    sched->appts[appt].x,
	    sched->appts[appt].y, sched->appts[appt].w, sched->appts[appt].h);
...
void Character::exec()
{
	...
	if (isResting()) {
		...
		if (clock_alarm_is_expired(&rest_alarm)) {
			if (isPlayerControlled() &&
			    vehicle && vehicle->getHp() < vehicle->getMaxHp()) {
				vehicle->heal(vehicle->getMaxHp() / 10);
				foogodRepaint();
	...
	if (isAsleep() && getActivity() != SLEEPING) {
		...
		if (ambushedWhileCamping && ((rand() % 100) < PROB_AWAKEN)) {
			endCamping();
		} else {
			endTurn();
			return;
	...
	case CONTROL_MODE_AUTO:
		...
		// action points.
		points_last_loop = 0;
		while (!isTurnEnded() && getActionPoints() != points_last_loop) {
	...
	case CONTROL_MODE_PLAYER:
		...
		if (PARTY_CONTROL_ROUND_ROBIN ==
		    player_party->getPartyControlMode()) {
...
void Character::save(struct save *save)
{
	...
	if (taskproc) {
		save->write(save, "(kern-char-task-continue kchar \"%s\" ",
			    getTaskName());
...
void Character::taskSetup(char *name_arg, struct closure *proc_arg,
			  struct gob *gob_arg)
{
...
void Character::taskBegin(char *name_arg, struct closure *proc_arg,
			  struct gob *gob_arg)
{
...
void Character::taskContinue(char *name_arg, struct closure *proc_arg,
			     struct gob *gob_arg)
{
	if (isPlayerControlled()) {
		log_msg("%s continues %s.", getName(), getTaskName());
