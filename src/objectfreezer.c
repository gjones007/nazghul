...
{
        objectfreezernode *newnode = (struct objectfreezernode *)
            malloc(sizeof (struct objectfreezernode));
        assert(newnode);
        newnode->treenode.key.s_key =
...
struct objectfreezerlist *new_objectfreezerlistnode(class Object * value, int x,
                                                    int y)
{
        objectfreezerlist *newnode = (struct objectfreezerlist *)
            malloc(sizeof (struct objectfreezerlist));
        assert(newnode);
...
class Object *freezer_thawObject(char *key, int *xout, int *yout)
{
        ...
        struct objectfreezerlist *ofln =
            (objectfreezerlist *) ofn->objectlist.next;
...
void freezer_save(save_t * save)
{
        ...
        for (ofntree = tree_minimum(ofntree); ofntree;
             ofntree = tree_successor(ofntree)) {
                struct objectfreezernode *ofn = (objectfreezernode *) ofntree;
                struct list *ofllist;
                list_for_each(&(ofn->objectlist), ofllist) {
                        struct objectfreezerlist *ofln =
                            (objectfreezerlist *) ofllist;
