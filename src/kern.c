...
static pointer kern_mk_palette(scheme * sc, pointer args)
{
        ...
        session_add(Session, Session->palette, terrain_palette_dtor, NULL,
                    NULL);
...
static pointer kern_mk_map(scheme * sc, pointer args)
{
        ...
        for (y = 0; y < height; y++) {
                ...
                for (x = 0; x < width; x++) {
                        ...
                        if (!tt && !palette_terrain_is_null_glyph(pal, glyph)) {
...
static pointer kern_mk_composite_map(scheme * sc, pointer args)
{
        ...
        for (y = 0; y < height; y++) {
                ...
                for (x = 0; x < width; x++) {
                        ...
                        terrain_map_blit(map, x * map->submap_w,
...
static int kern_place_load_subplaces(scheme * sc, pointer * args,
                                     struct place *place)
{
        pointer subplaces;
        ...
        while (scm_is_pair(sc, entrances)) {
                ...
                if (place_set_edge_entrance(place, dir, x, y)) {
                        load_err("kern-mk-place %s: failed to set entrance for "
                                 "direction %d to [%d %d]", place->tag, dir, x,
                                 y);
                        return -1;
                }
        }
...
KERN_API_CALL(kern_place_add_on_entry_hook)
{
        ...
        place_add_on_entry_hook(place, closure_new_ref(sc, proc));
        return sc->NIL;
}
...
static pointer kern_mk_species(scheme * sc, pointer args)
{
        ...
        species = species_new(tag, name, damage_sound, walking_sound, str,
                              ...
                              mpmod, mpmult, visible, n_slots, n_spells);
...
static pointer kern_mk_arms_type(scheme * sc, pointer args)
{
        ...
        arms = new ArmsType(tag, name, sprite, slots, hit, defend, hands,
                            ...
                            fire_sound, missile, ammo, str_attack_mod,
                            dex_attack_mod, char_damage_mod, char_avoid_mod,
                            false);
...
static pointer kern_mk_missile_type(scheme * sc, pointer args)
{
        ...
        if (unpack(sc, &args, "yspdopbb",
                   &tag,
                   &name,
                   &sprite, &gifc_cap, &gifc, &mmode, &beam, &fixedrange)) {
...
static pointer kern_mk_obj_type(scheme * sc, pointer args)
{
        ...
        if (unpack
            (sc, &args, "pddop", &sprite, &layer, &gifc_cap, &gifc, &mmode)) {
                load_err("kern-mk-obj-type %s: bad args (did you mean to use "
                         "kern-mk-obj instead?)", tag);
                return sc->NIL;
        }
...
KERN_API_CALL(kern_mk_player)
{
        ...
        if (unpack(sc, &args, "ypspdddppppp",
                   ...
                   &mv_desc, &mv_sound,
                   &food, &gold, &ttnm,
                   &form, &campsite, &camp_form, &vehicle, &inventory)) {
...
static pointer kern_mk_sched(scheme * sc, pointer args)
{
        ...
        for (i = 0; i < n_appts; i++) {
                ...
                if (unpack(sc, &p, "s", &activity)) {
                        load_err
                            ("kern-mk-sched %s: bad args in appt %d activity",
                             tag, i);
                        goto abort;
                }
...
static pointer kern_set_clock(scheme * sc, pointer args)
{
        if (unpack(sc, &args, "dddddd",
                   ...
                   &Session->clock.week,
                   &Session->clock.day,
                   &Session->clock.hour, &Session->clock.min)) {
...
static pointer kern_obj_get_activity(scheme * sc, pointer args)
{
        ...
        return scm_mk_string(sc, sched_activity_to_name(obj->getActivity()));
}
...
KERN_API_CALL(kern_place_is_visible)
{
        ...
        if (mapTileIsVisible(x, y)
            && (mapTileLightLevel(x, y) >= MIN_XAMINE_LIGHT_LEVEL)) {
                return sc->T;
        } else {
                return sc->F;
        }
...
static pointer kern_mk_astral_body(scheme * sc, pointer args)
{
        ...
        if (unpack(sc, &args, "ysdddddc",
                   ...
                   &distance,
                   &minutes_per_phase,
                   &minutes_per_degree, &initial_arc, &initial_phase, &proc)) {
        ...
        while (scm_is_pair(sc, phases)) {
                ...
                if (unpack(sc, &phase, "pds",
                           ...
                           &body->phases[i].maxlight, &phase_name)) {
...
KERN_API_CALL(kern_mk_vehicle_type)
{
        ...
        if (unpack(sc, &args, "yspppbbbspdddddpo",
                   ...
                   &tailwind_penalty,
                   &headwind_penalty,
                   &crosswind_penalty, &max_hp, &speed, &mmode, &proc)) {
        ...
        type = new VehicleType(tag,
                               ...
                               tailwind_penalty,
                               headwind_penalty,
                               crosswind_penalty, max_hp, speed);
        assert(type);
...
KERN_API_CALL(kern_map_view_center)
{
        ...
        if (!kern_unpack_loc(sc, &args, &place, &x, &y, "kern-map-view-center"))
                return sc->NIL;
...
KERN_API_CALL(kern_sound_play_at)
{
        ...
        if (unpack_loc
            (sc, &args, &place, &x, &y, "kern-sound-play-at: bad loc")) {
                return sc->NIL;
        }
...
KERN_API_CALL(kern_sound_play_ambient)
{
        ...
        if (unpack_loc
            (sc, &args, &place, &x, &y, "kern-sound-play-ambient: bad loc")) {
                return sc->NIL;
        }
        int volume = SOUND_MAX_VOLUME;
        int distance;
...
KERN_API_CALL(kern_mk_skill)
{
        ...
        if (unpack(sc, &args, "ddbbcc",
                   &skill->ap,
                   &skill->mp, &wilderness_ok, &passive, &yuse, &can_yuse)) {
...
KERN_API_CALL(kern_add_spell)
{
        ...
        if (!(spell = magic_add_spell(&Session->magic, code))) {
                load_err("kern-add-spell: failed to add %s", type->getName());
                return sc->NIL;
        }
...
KERN_API_CALL(kern_char_get_readied_weapons)
{
        ...
        for (weapon = ch->enumerateWeapons(&armsIndex); weapon != NULL;
             weapon = ch->getNextWeapon(&armsIndex)) {
                ...
                if (ch->species && weapon == ch->species->weapon)
                        continue;
...
KERN_API_CALL(kern_char_attack)
{
        ...
        ctrl_do_attack(attacker, weapon, defender, attacker->getToHitPenalty());
...
static void kern_ui_target_cleanup_info(struct kern_ui_target_info *info)
{
        ...
        while (entry != head) {
                struct location_list *tmp = (struct location_list *) entry;
                entry = entry->next;
...
KERN_API_CALL(kern_ui_target)
{
        ...
        place_for_each_object(Place, kern_ui_target_visitor, &info);
...
KERN_API_CALL(kern_fire_missile_to_max)
{
        ...
        /* Unpack the origin */
        if (unpack_loc
            (sc, &args, &oplace, &ox, &oy, "kern-fire-missile-to-max"))
                return sc->NIL;

        /* Unpack the destination */
        if (unpack_loc
            (sc, &args, &dplace, &dx, &dy, "kern-fire-missile-to-max"))
                return sc->NIL;
...
KERN_API_CALL(kern_char_set_player_controlled)
{
        ...
        ch = (class Character *) unpack_obj(sc, &args,
                                            "kern-char-set-player-controlled");
        if (!ch)
                return sc->NIL;
...
KERN_API_CALL(kern_being_get_visible_hostiles)
{
        ...
        return kern_place_for_each_object(sc, subj->getPlace(),
                                          kern_filter_visible_hostile, subj);
}
...
KERN_API_CALL(kern_being_get_visible_allies)
{
        ...
        return kern_place_for_each_object(sc, subj->getPlace(),
                                          kern_filter_visible_allies, subj);
}
...
KERN_API_CALL(kern_place_get_movement_cost)
{
        ...
        if (unpack_loc
            (sc, &args, &place, &x, &y, "kern-place-get-movement-cost"))
                goto done;
        ...
        for (pclass = 0; pclass < n_pclass; pclass++) {
                ...
                for (mmode = 0; mmode < n_mmode; mmode++) {
                        ...
                        if (unpack(sc, &col, "d", &mcost)) {
                                load_err
                                    ("kern-mk-ptable: row %d col %d bad arg",
                                     pclass, mmode);
                                goto abort;
                        }
        ...
        for (r_faction = 0; r_faction < n_factions; r_faction++) {
                ...
                if (scm_len(sc, row) < n_factions) {
                        load_err("kern-mk-dtable: row %d has only %d columns "
                                 "(expected %d)",
                                 r_faction, scm_len(sc, row), n_factions);
                        goto abort;
                }
...
static pointer kern_dtable_aux(scheme * sc, pointer args, const char *name,
                               int fx)
{
        int f1, f2, level;
...
KERN_API_CALL(kern_party_add_member)
{
        ...
        party =
            (Party *) unpack_obj(sc, &args, "kern_party_add_member:<party>");
        if (!party)
                return sc->NIL;
...
KERN_API_CALL(kern_being_set_base_faction)
{
        ...
        being =
            (class Being *) unpack_obj(sc, &args,
...
KERN_API_CALL(kern_being_set_current_faction)
{
        ...
        being =
            (class Being *) unpack_obj(sc, &args,
                                       "kern-being-set-current-faction");
        if (!being)
                goto done;
...
KERN_API_CALL(kern_vehicle_set_name)
{
        ...
        vehicle =
            (class Vehicle *) unpack_obj(sc, &args, "kern-vehicle-set-name");
        if (!vehicle)
                goto done;
...
KERN_API_CALL(kern_being_get_current_faction)
{
        ...
        being =
            (Being *) unpack_obj(sc, &args, "kern-being-get-current-faction");
        if (!being)
                goto done;
...
KERN_API_CALL(kern_add_query)
{
        ...
        for (id = 0; id < NUM_HOOKS; id++) {
                if (!strcmp(query_to_id[id], str)) {
                        session_add_query(Session, (session_query_id_t) id,
                                          closure_new(sc, pproc));
                        return pproc;
                }
        }
...
static pointer kern_astar_path_to_scheme_list(scheme * sc,
                                              struct astar_node *path)
{
        ...
        cell = _cons(sc,
                     ...
                     scm_mk_integer(sc, path->y), 0);

        /* recursively build a scheme list of pairs */
        head = _cons(sc,
                     cell, kern_astar_path_to_scheme_list(sc, path->next), 0);
...
static pointer kern_build_weapon_list(scheme * sc,
                                      class Character * character,
                                      class ArmsType * weapon, int *armsIndex)
{
        /* base case */
        if (!weapon)
                return sc->NIL;
 ...
 * A generic append-to-scheme-list function.
 */
static void kern_list_append_pointer(struct kern_append_info *info,
                                     pointer cell)
{
        /* make it a list element */
        cell = _cons(info->sc, cell, info->sc->NIL, 0);
        ...
        while (scm_is_pair(sc, args)) {
                ...
                if (*line == '\0') {
                        pointer paginated_string_element =
                            scm_mk_string(sc, line);
                        kern_list_append_pointer(&info,
...
KERN_API_CALL(kern_char_get_skills)
{
        ...
        /* add species skills */
        if (subj->species && subj->species->skills) {
                kern_add_skill_set(&info, subj->getLevel(),
                                   subj->species->skills);
        }

        /* add occupation skills */
        if (subj->occ && subj->occ->skills) {
                kern_add_skill_set(&info, subj->getLevel(), subj->occ->skills);
        }
...
static pointer kern_build_arm_list(scheme * sc,
                                   class Character * character,
                                   class ArmsType * arm, int *armsIndex)
{
        /* base case */
        if (!arm)
                return sc->NIL;
...
KERN_API_CALL(kern_obj_get_hp)
{
        ...
        /* unpack the character */
        kobj = (class Object *) unpack_obj(sc, &args, "kern-obj-get-hp");
        if (!kobj)
                return sc->NIL;
...
KERN_API_CALL(kern_char_get_level)
{
        ...
        /* unpack the character */
        character =
            (class Character *) unpack_obj(sc, &args, "kern-char-get-level");
        if (!character)
                return sc->NIL;
...
KERN_API_CALL(kern_char_set_level)
{
        ...
        /* unpack the character */
        character =
            (class Character *) unpack_obj(sc, &args, "kern-char-set-level");
        if (!character)
                return sc->NIL;
...
KERN_API_CALL(kern_char_get_strength)
{
        ...
        /* unpack the character */
        character =
            (class Character *) unpack_obj(sc, &args, "kern-char-get-strength");
        if (!character)
                return sc->NIL;
...
KERN_API_CALL(kern_char_set_strength)
{
        ...
        /* unpack the character */
        character =
            (class Character *) unpack_obj(sc, &args, "kern-char-set-strength");
        if (!character)
                return sc->NIL;
...
KERN_API_CALL(kern_char_get_speed)
{
        ...
        /* unpack the character */
        character =
            (class Character *) unpack_obj(sc, &args, "kern-char-get-speed");
        if (!character)
                return sc->NIL;
...
KERN_API_CALL(kern_arms_type_fire_in_direction)
{
        ...
        if (unpack_loc
            (sc, &args, &place, &startx, &starty,
             "kern_arms_type_fire_in_direction"))
                return sc->NIL;
...
KERN_API_CALL(kern_search_rect_for_terrain)
{
        ...
        /* unpack the args */
        if (unpack(sc, &args, "pddddp", &place, &ulc_x, &ulc_y, &w, &h, &ter)) {
...
KERN_API_CALL(kern_begin_combat)
{
         ...
         * vector. */
        if ((!info.dx && !info.dy) || (info.dx && info.dy)) {
...
KERN_API_CALL(kern_ambush_while_camping)
{
        ...
        party =
            (class Party *) unpack_obj(sc, &args, "kern-ambush-while-camping");
        if (!party)
                return sc->F;
...
static void kern_ztats_pane_enter(struct ztats_pane *pane, class Party * party,
...
static int kern_ztats_pane_scroll(struct ztats_pane *pane,
...
KERN_API_CALL(kern_ztats_add_pane)
{
        ...
        if (unpack
            (sc, &args, "ooool", &penter, &pscroll, &ppaint, &pselect, &pgob)) {
...
static void kern_applet_run(struct applet *applet, SDL_Rect * dims,
...
KERN_API_CALL(kern_script_version)
{
        ...
        if (unpack(sc, &args, "s", &verstr)) {
                ...
                sprintf(buffer, "%u.%u.%u", Session->major, Session->minor,
                        Session->release);
...
scheme *kern_init(void)
{
        ...
        /* kern-arms-type api */
        API_DECL(sc, "kern-arms-type-get-ammo-type",
                 kern_arms_type_get_ammo_type);
        ...
        API_DECL(sc, "kern-arms-type-fire-in-direction",
                 kern_arms_type_fire_in_direction);
        ...
        API_DECL(sc, "kern-being-get-visible-tiles",
                 kern_being_get_visible_tiles);
        ...
        API_DECL(sc, "kern-being-set-current-faction",
                 kern_being_set_current_faction);
        ...
        API_DECL(sc, "kern-obj-remove-from-inventory",
                 kern_obj_remove_from_inventory);
        ...
        API_DECL(sc, "kern-obj-set-ignore-time-stop",
                 kern_obj_set_ignore_time_stop);
        ...
        /* kern-place api */
        API_DECL(sc, "kern-place-add-on-entry-hook",
                 kern_place_add_on_entry_hook);
        API_DECL(sc, "kern-place-apply-tile-effects",
                 kern_place_apply_tile_effects);
        ...
        API_DECL(sc, "kern-place-get-movement-cost",
                 kern_place_get_movement_cost);
        ...
        API_DECL(sc, "kern-place-move-is-passable?",
        ...
        API_DECL(sc, "kern-terrain-set-combat-map",
                 kern_terrain_set_combat_map);
        API_DECL(sc, "kern-terrain-set-combat-handler",
        ...
        API_DECL(sc, "kern-type-set-quest-item-flag",
                 kern_type_set_quest_item_flag);
        ...
        API_DECL(sc, "kern-set-magic-negated-sprite",
                 kern_set_magic_negated_sprite);
        API_DECL(sc, "kern-set-reveal-sprite", kern_set_reveal_sprite);
        API_DECL(sc, "kern-set-xray-vision-sprite",
                 kern_set_xray_vision_sprite);
        ...
        API_DECL(sc, "kern-sprite-append-decoration",
                 kern_sprite_append_decoration);
        API_DECL(sc, "kern-sprite-get-tag", kern_sprite_get_tag);
        API_DECL(sc, "kern-sprite-strip-decorations",
                 kern_sprite_strip_decorations);
        ...
        scm_define_int(sc, "kern-key-up", SDLK_KP_8);    // also handles arrowkeys
        scm_define_int(sc, "kern-key-down", SDLK_KP_2);  // also handles arrowkeys
        scm_define_int(sc, "kern-key-kp-pgup", SDLK_KP_9);
        scm_define_int(sc, "kern-key-kp-pgdn", SDLK_KP_3);
