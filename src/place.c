...
Object *place_get_lower_object(struct place * place, int x, int y,
                               enum layer layer, int *depth)
{
        struct tile *tile;
        ...
        /* Increment depth if caller wants it. */
        if (depth) {
                (*depth)++;
        }

        /* Recur. */
        return place_get_lower_object(lower, x, y, layer, depth);
}
...
int place_get_light(struct place *place, int x, int y)
{
        ...
        struct terrain *ter = place_get_terrain(place, x, y);
        light = ter ? ter->light : 0;
                                        ...
                                        int *depth)
{
        ...
        /* Recur. */
        return place_get_lower_terrain(lower, x, y, depth);
}

static void place_describe_terrain(struct place *place, int x, int y)
{
        int depth = 0;
        struct terrain *t = place_get_lower_terrain(place, x, y, &depth);
        log_continue("%s", t ? t->name : "nothing");
        if (depth) {
                log_continue(" (%d level%s down)", depth, depth > 1 ? "s" : "");
        }
        if (DeveloperMode && t) {
                log_continue("[tag=%s alpha=%d sprite=%s]", t->tag, t->alpha,
                             sprite_get_tag(t->sprite));
        }
}

/* Fwd decl for mutual recursion  */
                                        ...
                                        int is_first)
{
        if (!place_get_terrain(place, x, y)) {
                ...
                lower = place_get_neighbor(place, DOWN);
                if (lower) {
                        return place_describe_objects(lower, x, y, is_first);
                }
        }
        return 0;
}
                                  ...
                                  int is_first)
{
        ...
        tile = place_lookup_tile(place, x, y);
        if (!tile) {
                return place_describe_lower_objects(place, x, y, is_first);
        }
        ...
        if (n_types == 0) {
                /* Nothing to list at this level, try the next level down. */
                return place_describe_lower_objects(place, x, y, is_first);
        }
...
static void place_examine_objects(struct place *place, int x, int y)
{
	...
	if (!found) {
		...
		if (!place_get_terrain(place, x, y)) {
			...
			lower = place_get_neighbor(place, DOWN);
			if (lower) {
				place_examine_objects(lower, x, y);
			}
		}
	}
}
...
void place_exec(struct place *place)
{
        ...
        while (place->turn_elem != &place->turn_list
               ...
               && !Reload
#if ! CONFIG_CONCURRENT_WILDERNESS
               && place == Place        /* otherwise projectiles and damage
                                         * flashes from the old place are shown
                                         * in the new place until the turn is
                                         * over when the player does a
                                         * switch */
#endif
            ) {
