...
bool Party::attackPlayer(int dx, int dy)
{
	...
	if (dx && dy) {
		int xgoodness = 0;
		int ygoodness = 0;
		if (place_is_passable(getPlace(), getX() + dx, getY(), this, 0)) {
		...
		if ((xgoodness < 2)
		    && place_is_passable(getPlace(), getX(), getY() + dy, this,
					 0)) {
			if (place_is_hazardous(getPlace(), getX() + dx, getY())) {
				ygoodness = 1;
			} else {
	...
	if (join_existing_combat) {
		...
		dbg("attackPlayer:%s:joining the combat at %s\n", getName(),
		    combat_place->name);
		if (combat_add_party_on_edge
		    (this, info.dx, info.dy, combat_place)) {
...
void Party::forEachReverseMember(bool(*fx) (class Character *, void *),
				 void *data)
{
	struct node *elem;
...
void Party::distributeMembers()
{
	...
	combat_fill_position_info(&pinfo, getPlace(), getX(), getY(), dx, dy,
				  false);
...
void Party::setPlace(struct place *place)
{
	...
	if (this != player_party && !place_is_wilderness(place)) {
...
Object *Party::getSpeaker()
{
	...
	statlist =
	    (struct stat_list_entry *)calloc(getSize(),
					     sizeof(struct stat_list_entry));
	assert(statlist);
